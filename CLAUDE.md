...
**Segment formatting quirks live in the segment files.** A few non-obvious behaviors worth knowing before touching them: branch names are capped at `maxBranchWidth` (28 cols) with a middle ellipsis (`render.Truncate`); model names are compacted by `shortenModel` (e.g. `Opus 4.8 (1M context)` → `Opus 4.8(1M)`); the `lines` segment shows working-tree changes vs the last commit (`git diff HEAD --numstat` for tracked files, **plus** untracked non-ignored files counted as added via `untrackedAddedLines` so the total matches Claude Code's own counter) and hides itself again after a commit, rendering its `+N`/`-N` as one flush two-tone pill in `blocks` style.
