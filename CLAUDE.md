...
**The catalog is the source of truth for discovery.** `internal/segments/catalog.go` holds `[]Meta` (description, `DefaultOn`, `RequiresGit`, `ProMaxOnly`, `Role`). The guided-setup skill and `cosmobar segments --json` read this dynamically — nothing about segments is hardcoded in the UI. `TestCatalogMatchesRegistry` fails the build if a registered segment lacks a catalog entry or vice versa, so the two must stay in sync.
